...
@Component({
  selector: 'app-renta-variable3',
  templateUrl: './renta-variable3.component.html',
  styleUrls: ['./renta-variable3.component.css']
})
export class RentaVariable3Component implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.slider = this.document.querySelector('#sliderInver');
    this.fill = this.document.querySelector('.barInver .fillInver');
    
    this.slider2 = this.document.querySelector('#sliderInver2');
    this.fill2 = this.document.querySelector('.barInver2 .fillInver2');

    this.graf = this.document.querySelector('#gra2');
  ...
  setBar2() {
    ...
    for(let i=1; i<=30; i++) {
      if(this.slider2.value==i) {
        this.graf.setAttribute('src',`assets/img/simulador/${i}.svg`);
      }
    }
